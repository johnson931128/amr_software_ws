...
## 💻 3. 標準啟動流程 (Quick Start Guide)

每次打開全新的 WSL 2 終端機時，請嚴格按照以下順序執行，確保環境變數載入與編譯正確。

### Step 0: 編譯與環境載入 (每次修改程式碼後必做)
打開第一個終端機（Terminal 1），回到工作區根目錄並編譯：
```bash
cd ~/amr_ws
...
### Step 2: 啟動大腦決策層 (Terminal 2)

開啟第二個全新的 WSL 2 終端機分頁（用來發送控制指令），同樣需要先載入環境變數，接著執行 C++ 測試節點：
```bash
cd ~/amr_ws
source install/setup.bash
ros2 run amr_mission_control square_tester
```
...
## 🎯 4. 總指揮自動化架構：`run_square.launch.py` (Top-Level Launch)

為達到最終實機部署等級的「一鍵全自動啟動」，此腳本負責協調物理模擬與大腦邏輯的啟動時序（Sequencing）：
* **模組化呼叫 (`IncludeLaunchDescription`)**：直接引入已封裝好的 `gazebo.launch.py`，將車體與 3D 世界的生成視為一個獨立的黑盒子模組。
* **時序控制 (`TimerAction`)**：考量到 Gazebo 物理引擎載入 3D 資源需要時間，刻意將 C++ 決策大腦 (`square_tester`) 的啟動時間延遲 3 秒。這能有效避免大腦提早發送 `/cmd_vel` 指令，導致車體漏掉初始動作的非預期狀況。這也是大型機器人軟體架構中確保各節點穩定啟動的標準做法。
